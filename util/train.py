...
def train(
    ...
    if model.predictor:
        params.append({"params": model.predictor.parameters(), "lr": lr_vqa})
    if model.generator:
        params.append({"params": model.generator.parameters(), "lr": lr_cap})
    ...
    for epoch in range(start_epoch, num_epoches):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
            ...
            if i % checkpoint == 0 and i != 0:
                ...
                logger.write(
                    f"[Batch {i}] loss: {(avg_loss-prev_loss)/checkpoint:.4f} ({t})"
                )
                prev_loss = avg_loss
        ...
        if model.predictor != None:
            ...
            t = time.strftime("%H:%M:%S", time.gmtime(time.time() - start))
            logger.show(
                f"[Epoch {epoch}] avg_loss: {avg_loss:.4f} | score: {eval_score:.10f} ({t})"
            )
            ...
            if eval_score > best_score:
                ...
                best_epoch = epoch
            logger.show(
                f"[Result] best epoch: {best_epoch}, score: {best_score:.10f} / {bound:.10f}"
            )
        else:
            avg_loss /= batches
...
def fine_tune(
    ...
    for epoch in range(start_epoch, num_epoches):
        ...
        t = time.strftime("%H:%M:%S", time.gmtime(time.time() - start))
        logger.write(
            f"[Epoch {epoch}] loss: {(avg_loss-prev_loss)/checkpoint:.4f} ({t})"
        )
        prev_loss = avg_loss
        avg_loss = 0
...
def evaluate(
    model, dataloader, logger=None, writer=None, ans_index=None, save_path=None
):
    """
    ...
    if ans_index is not None:
        ...
        for ans in ans_index:
            output["hparam/" + ans] = all_score[ans_index[ans]].sum() / len(
                ans_index[ans]
            )
        for i in output:
