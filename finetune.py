...
def parse_args():
    ...
    parser.add_argument('--load_epoch', type=int, default=-1, help='load certain epoch (default = -1 means select the best epoch)')
...
def main():
    ...
    name = f'epoch_{main_args.load_epoch}.pt' if main_args.load_epoch != -1 else 'best_model.pt'
    ...
    encoder = set_model(encoder_type=args.encoder_type,
                        decoder_type='none',
                        predictor_type='none',
    ...
    predictor = set_model(  predictor_type=args.predictor_type,
                            decoder_type='none',
                            encoder_type='none',
                            ntoken=len(vocab_list),
                            v_dim=args.v_dim,
    ...
    print('model ready.')
    if args.predictor_type != 'att-cap': caption_path = ''
    else:
    ...
    if main_args.mode in ['train', 'val']:
        ...
        for epoch in range(main_args.epoches):
            ...
            logger.show(f'[Epoch {epoch}] score: {score:.10f} / bound: {target_score:.10f}')
            writer.add_scalar('val/vqa/score', score, epoch)

            if best_score < score:
                best_score = score
                best_epoch = epoch
                torch.save(predictor.state_dict(), os.path.join(save_path, 'best_epoch.pt'))
        ...
        name = 'best_epoch.pt' if main_args.load_predictor == -1 else f'epoch_{main_args.load_predictor}.pt'
        param = torch.load(
