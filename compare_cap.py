...
def main(my_args):
    ...
    files = sorted(
        [
            file
            for file in os.listdir(os.path.join("checkpoint", my_args.exp_name))
            if file.endswith(".pt") and file != "best_model.pt"
        ]
    )
    for file in files:
        model.load_state_dict(
            torch.load(
                os.path.join("checkpoint", my_args.exp_name, file),
        ...
        bleu = list(
            get_score(
                references, hypotheses, meteor=False, cider=False, rouge=False
            ).values()
        )
        score = sum(bleu) / len(bleu)
        str_bleu = ""
        for i in bleu:
            str_bleu = str_bleu + f"{i:.4f}/"
        ...
        if best_score < score:
            ...
            torch.save(
                model.state_dict(),
                os.path.join("checkpoint", my_args.exp_name, "best_epoch.pt"),
            )
        logger.write(msg)
