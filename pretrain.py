...
def parse_args():
    ...
    parser.add_argument('--load_path', type=str, default='../annot/VQA-E', help='path for loading dataset')
    ...
    parser.add_argument('--load_finetune_model', type=str, default='', help='path for the fine-tuned model to evaluate')
    parser.add_argument('--load_caption', type=str, default='', help='path for the generated captions')
    ...
    parser.add_argument('--start_batch', type=int, default=0, help='the previous batch number if need to train continuosly')
...
def main():
    ...
    if args.mode == 'val' or args.mode == 'vqa-val':
        ...
        if args.load_model == '':args.load_model = 'best_model.pt'
        model.load_state_dict(torch.load(os.path.join('checkpoint', args.comment, args.load_model)))
        print('load parameters: ', args.load_model)
        if args.load_finetune_model != '':
            model.load_state_dict(
                torch.load(os.path.join('checkpoint', args.comment, args.load_finetune_model), map_location=args.device),
                strict=False
            )
        if args.load_caption != '': args.load_caption = os.path.join('checkpoint', args.comment, args.load_caption)
    ...
    if 'decode' in args.mode:
        if args.load_model == '': args.load_model = 'best_model'
        ...
        if args.save_path == '': args.save_path = f'{args.load_model}.txt'
        with open(os.path.join(save_path, 'decode', args.save_path), 'w') as f:
            for i, batch in enumerate(tqdm(dataloader)):
                if args.start_batch > i: continue
                if args.batches == i and i != 0: break
