...
def ce_for_language_model(caption, alpha=1.):
...
class Wrapper(nn.Module):
    def __init__(self, encoder=None, predictor=None, generator=None, use_mtl=True, device=''):
        super().__init__()
    ...
    def forward(self, batch):
        ...
        if self.generator is None and self.predictor is None: return batch
    ...
    def get_loss(self, batch):
        ...
        if predict is not None:
            ...
            writes['train/score'] = compute_score(predict, target, self.device).sum().item() / target.size(0)
    ...
    def forward_vqa(self, batch):
        target = batch['a'].float().to(self.device)
        if self.encoder is not None: batch = self.encoder(batch)
        predict = self.predictor(batch)
