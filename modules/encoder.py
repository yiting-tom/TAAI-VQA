...
class BaseEncoder(nn.Module):
    ...
    def base_forward(self, batch):
        ...
        if 'c' in batch:
            ...
            output['c'] = self.embedding(output['c_target']) # [batch, c_len, embed_dim]
        return output
    ...
    def forward(self, batch):
        ...
        output['v_att'] = self.attention(output['v'], output['q']) # [batch, num_objs, 1]
...
class RelationEncoder(BaseEncoder):
    ...
    def forward(self, batch, show_att=False):
        ...
        graph = torch.zeros_like(batch['graph']).unsqueeze(3).repeat(1,1,1,self.label_num)
        ...
        # Relation encoder
        output_v = self.spatial_encoder(output['v'], graph, show_att=show_att) # [batch, num_objs, v_dim]
        if show_att: return output_v
        output['v'] = output_v
        return output
